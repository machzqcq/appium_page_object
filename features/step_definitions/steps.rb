...
Given /^I have entered (\d+) into field1 of the calculator$/ do |value|
  first_element = selenium.find_element(:name, "TextField1")
  first_element.send_keys value
end

And /^I have entered (\d+) into field2 of the calculator$/ do |value|
  first_element = selenium.find_element(:name, "TextField2")
  first_element.send_keys value
end
...
Then /^the result should be displayed as (\d+)$/ do |expected|
  ...
  expect(result.attribute("value")).to eql(expected)
end
